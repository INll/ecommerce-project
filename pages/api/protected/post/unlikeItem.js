...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      await User.updateOne({ userName: payload.payload.userName }, { $pull : { favItems: req.body.item._id } })
      let updatedUser = await User.findOne({ userName: payload.payload.userName}).exec();
      return res.status(200).json({ message: `${req.body.item.title} has been removed from favourites!`, user: updatedUser, result: 1 });
    } else {
